...
def process_sentence(line):
    return list(filter(lambda x: x in string.punctuation,
                       line.translate(str.maketrans('', '', '!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~' + '–')).split()))


def process_alignment(line):
    return list(map(lambda x: tuple(x.split('-')), line.split()))
...
def extract_sentences(filename: str) -> Tuple[List[SentencePair], List[LabeledAlignment]]:
    ...
    for s in tree.getroot():
        sentence_pairs.append(SentencePair(process_sentence(s[0].text),
                                           process_sentence(s[1].text)))
        alignments.append(LabeledAlignment(process_alignment(s[2].text),
                                           process_alignment(s[3].text)))
