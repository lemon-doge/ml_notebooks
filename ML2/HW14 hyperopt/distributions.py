...
class LogUniformDistribution(NumericalDistribution):
    ...
    def sample(self, size: int) -> np.array:
        return np.exp(np.random.uniform(self.log_low, self.log_high, size))

    def scale(self, sample: np.array) -> np.array:
        return (np.log(sample) - self.log_low) / (self.log_high - self.log_low)
...
class IntLogUniformDistribution(NumericalDistribution):
    ...
    def sample(self, size: int) -> np.array:
        return np.exp(np.random.uniform(self.log_low, self.log_high, size)).astype(int)
